...
export const loginWithPrivateKey = async (privateKeyInput) => {
  try {
    ...
    console.log('Input key:', privateKeyInput);
    console.log('Key length:', privateKeyInput.length);
    console.log('Key type:', typeof privateKeyInput);
    
    let sk;
    let privateKeyHex;
    
    // Try to decode as nsec1
    if (privateKeyInput.startsWith('nsec1')) {
      console.log('Detected nsec1 format, attempting decode...');
      try {
        // Try different decode methods
        let decoded;
        
        // Method 1: Direct decode
        decoded = nip19.decode(privateKeyInput);
        ...
        if (decoded && decoded.data) {
          ...
          console.error('Decode returned null or no data');
          return null;
        }
      } catch (decodeError) {
    ...
    // Try as hex
      ...
      console.error('Unknown key format - does not start with nsec1 and is not 64 char hex');
      return null;
    }
